          ...
          request.getRequestDispatcher( "/mantenimientos/listadoAreas.jsp" ).forward( 
            ...
            request.getRequestDispatcher( "/mantenimientos/editarAreas.jsp" ).forward( 
             request,response );
                ...
                if (request.getParameter("boton").equals("NuevoEliminarAlumno")) {
                ...
                if (nroDoc != null && !nroDoc.isEmpty()) {
                    List<Matricula> matriculas = dao.buscarMatriculasPorNroDoc(nroDoc);
                    if (!matriculas.isEmpty()) {
                        out.println("<h2>Matrículas encontradas:</h2>");
                        for (Matricula matricula : matriculas) {
                            out.println("<h2>Detalles de la Matrícula:</h2>");
                            out.println("Código: " + matricula.getCodigo());
                            out.println("Fecha: " + matricula.getFecha());
                            out.println("Número de Documento: " + matricula.getNro_doc());
                            out.println("Código del Alumno: " + matricula.getCodigo_alumno());
                            out.println("Total: " + matricula.getTotal());
                            out.println("Estado: " + matricula.getEstado());
                            if (matricula.getNro_doc()!= null) {
                                out.println("Nombre del Alumno: " + matricula.getNro_doc());
                            }
                    }
                        request.setAttribute("matriculas", matriculas);
                        request.getRequestDispatcher("/mantenimientos/listarMatriculas.jsp").forward(
                            request, response);
                } else {
                        out.println("<h2>Matrícula no encontrada.</h2>");
                    }
                } else {
                    List<Matricula> matriculas = dao.listarMatriculas();
                    if (!matriculas.isEmpty()) {
                    ...
                    request.setAttribute("matriculas", matriculas);
                    request.getRequestDispatcher("/mantenimientos/listarMatriculas.jsp").forward(request, response);
